...
export default class SfGpsDsCaOnSearch extends SfGpsDsLwc {
    ...
    constructor() {
        ...
        this._debouncedEmitUpdate = debounce((value) => this.emitUpdateEvent(value), this.debounceMs || 300);
    }
    /* event handlers */
    handleInput(event) {
    ...
    scrollActiveIntoView() {
        if (this._activeIndex < 0)
            return;
        const activeOption = this.querySelector(`[data-index="${this._activeIndex}"]`);
        if (activeOption) {
